...
给其他 **命令传递参数** 的一个过滤器，也是组合多个命令的一个工具。它擅长将标准输入数据转换成命令行参数，xargs 能够处理管道或者 stdin 并将其转换成特定命令的命令参数。xargs 也可以将单行或多行文本输入转换为其他格式，例如多行变单行，单行变多行。xargs 的默认命令是 echo，空格是默认定界符。这意味着通过管道传递给 xargs 的输入将会包含换行和空白，不过通过 xargs 的处理，换行和空白将被空格取代。xargs 是构建单行命令的重要组件之一。
    ...
    xargs [选项] [参数]
        选项：
            ...
            -I  可以自定义替换字符串，这个字符串在xargs扩展时会被替换掉，当-I与xargs结合使用，每一个参数命令都会被执行一次：
            -i  告诉 xargs 可以使用{}代替传递过来的参数， 建议使用-I，其符合POSIX标准
            -L  
            -l  与-n选项类似，唯一的区别是-L永远是按段划批，而-n在和独立的xargs一起使用时是按空格分段划批的
            ...
            -r   如果没有要处理的参数传递给xargs,xargs 默认是带空参数运行一次，如果你希望无参数时，停止 xargs，直接退出，使用 
            -s  传入最大字符长度限制，包括命令与参数
            -t  打开调试功能，显示每次所组的命令，在调试中非常有用，可以看出xargs的执行原理
...
```shell
[root@localhost tom]# ls *.md | xargs wc -l
  202 a.md
  208 b.md
  202 ces.md
  612 total
...
```shell
[root@localhost tom]#  ls *.md | xargs -n 2 -p wc -l
wc -l a.md b.md ?...y
  202 a.md
  208 b.md
  410 total
